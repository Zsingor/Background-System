...
public interface RolesRoutesMapper {
    ...
    @Select("select routeid from sys_data.roles_routes where roleid=#{id}")
    List<String> queryRoleRoutes(Roles roles);

    //删除某个角色所有对应的路由信息
    @Delete("delete from sys_data.roles_routes where roleid=#{id} and routetype='1'")
    void deleteRoleRoutes(Roles roles);

    //删除某个角色所有对应的权限信息
    @Delete("delete from sys_data.roles_routes where roleid=#{id} and routetype='2'")
